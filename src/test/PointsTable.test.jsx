...
describe('PointsTable', () => {
  ...
  it('displays all players with correct stats', () => {
    ...
    expect(screen.getAllByText('3').length).toBeGreaterThanOrEqual(4); // Charlie and David's points, plus Thru values and other stats
  });

  it('calculates "Thru" values correctly', () => {
    render(<PointsTable players={mockPlayers} currentHole={4} />);
    
    // All players should show "3" for Thru (currentHole - 1)
    const thruValues = screen.getAllByText('3');
    // Should have 4 instances of "3" for Thru values, plus Charlie and David's points
    expect(thruValues.length).toBeGreaterThanOrEqual(4);
  });
